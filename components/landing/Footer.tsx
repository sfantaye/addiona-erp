      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12 items-start">
          <div className="col-span-1 md:col-span-2 lg:col-span-1" data-aos="fade-up">
            <Link href="/" className="flex items-center mb-4">
                <Zap className="h-8 w-8 text-[var(--color-addiona)] mr-2" />
            ...
            <div className="flex space-x-4">
              {socialLinks.map(social => (
                <a key={social.label} href={social.href} aria-label={social.label} target="_blank" rel="noopener noreferrer"
                   className="text-addiona-gray-300 hover:text-[var(--color-addiona)] transition-colors duration-300 transform hover:scale-110">
                  {social.icon}
                </a>
              ))}
            </div>
             ...
             <div key={section.title} data-aos="fade-up" data-aos-delay={`${(idx + 1) * 100}`}>
                ...
                <ul className="space-y-2">
                  {section.links.map(link => (
                    <li key={link.name}>
                      <Link href={link.href} className="hover:text-[var(--color-addiona)] transition-colors duration-300">
                        {link.name}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="relative my-8" data-aos="zoom-in">
            ...
            <div className="relative flex justify-center">
                <span className="bg-black px-4 text-[var(--color-addiona)]">
                    <Zap size={20} />
                </span>
            </div>
        </div>
        ...
        <div className="text-center text-sm pt-8" data-aos="fade-up" data-aos-delay="500">
          ...
          <div className="mt-2">
            <Link href="/privacy-policy" className="hover:text-[var(--color-addiona)] transition-colors px-2">Privacy Policy</Link>
            <span className="text-gray-600">|</span>
            <Link href="/terms-of-service" className="hover:text-[var(--color-addiona)] transition-colors px-2">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
